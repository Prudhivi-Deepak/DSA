class Solution
{
    public:
        int f(int i, int remW, vector<int>& wt, vector<int>& val, vector<vector<int>> &dp){
            ...
            int Take = 0;
            if(remW>=wt[i]) Take = val[i] + f(i, remW-wt[i], wt, val, dp);
        ...
        int unboundedKnapsack(vector<int>& wt, vector<int>& val, int n, int W) {
            ...
            // i goes from n-1 to 0 in f 
            // base case: for any remW dp[0][remW] = (remW*val[0])/wt[0]

            for(int remW=0; remW<=W; remW++){
                dp[0][remW] = (remW/wt[0])*val[0];
            }
            ...
            for(int i=1; i<n; i++){
                for(int remW=0; remW<=W; remW++){
                    int NotTake = dp[i-1][remW];
                    int Take = 0;
                    if(remW>=wt[i]) Take = val[i] + dp[i][remW-wt[i]];
