class Solution{
public:
    int f(int buyNow, int i, vector<int> arr, int n, vector<vector<int>> &dp){
        if(i>=n) return 0;

        if(dp[i][buyNow] != -1) return dp[i][buyNow];

        int notTake = f(buyNow, i+1, arr, n, dp);
        int take = 0;
        if(buyNow){
            take = -arr[i] + f(!buyNow, i+1, arr, n, dp);
        }
        else{
            take = arr[i] + f(!buyNow, i+1, arr, n, dp);
        }
